...
http.interceptor.response = (response) => {
	...
	if (response.errMsg == 'request:ok' || response.errMsg == 'uploadFile:ok') {
		...
		if (code == 400) {
			if(isSaveOrderUrl.indexOf("saveOrderList") == -1&&(isSaveOrderUrl.indexOf("saveOrder") != -1||isSaveOrderUrl.indexOf("updateOrder") != -1||isSaveOrderUrl.indexOf("getBlProductType") != -1||isSaveOrderUrl.indexOf("saveOrderList") != -1)){
				console.log('使用自定义提示~')
			}else{
				setTimeout(() => {
					uni.showToast({
...
// 是否允许时效件
export const getBlProductType = (data) => {
	return http.request({
		url: '/order/getBlProductType',
		method: "GET",
		data
	})
}
// 是否允许时效件
export const checkDispByCity = (data) => {
	return http.request({
		url: '/order/checkDispByCity',
		method: "GET",
		data
	})
}
