			...
			<view class="track_wraper" v-for="(item,index) in trackData" :key="index">
				...
				<view class="express_word">
					...
					<view class="express_word_text">
						...
						<view class="view-p" v-if="item.scanType=='取消拦截'">快件在<text
								class="mgr_5">【{{item.scanSite||''}}】</text>进行取消拦截操作
						</view>
						<view class="view-p" v-if="item.scanType=='取消退件'">快件在<text
								class="mgr_5">【{{item.scanSite||''}}】</text>进行取消退件操作
						</view>
						<view class="view-p" v-if="item.scanType=='运单取消'">快件在<text
								class="mgr_5">【{{item.scanSite||''}}】</text>进行运单取消操作
						</view>
					</view>
				</view>
			</view>
