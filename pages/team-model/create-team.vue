<template>
	...
	<view class="createTeam">
		...
		<view class="createTeam-info">
			<view class="createTeam-info-head" :class="myTeamInfo.status==0||myTeamInfo.blPaymentType==1?'mb62':''">
				<view>{{teamInfo.groupName||''}}（{{memberList.length||''}}）</view>
				<view>团队号: {{teamInfo.groupId||''}}</view>
				<view class="width100" v-if="myTeamInfo.status!=0&&myTeamInfo.blPaymentType!=1">面单余额: <text  class="text-weight" > {{myTeamInfo.quantity||0}}</text> </view>
			</view>
