		...
		<view class="courier" v-if="teamModel!='1'">
			<view class="l">
				...
				<view class="text uni-ellipsis" >
					<text>{{specEmployee||'暂未绑定专属小哥'}}</text>
					<text>{{ownerSite}}</text>
				</view>
			</view>
			<view class="r" @click="editMycourier" v-if="!isEdit">
				...
				<view class="text">
					{{specEmployee?'去选择':'去绑定'}}
				</view>
			</view>
		</view>
<!-- 		<view class="connect-ble" v-if="!isEdit">
			<view class="l">
				<view class="imgBox">
					<image src="../../../static/img/headImg.png" mode=""></image>
				</view>
				<view class="text " >
					<text v-if="GET_CONNECT_DEVICEId">当前设备:</text>
					<text class="bleName uni-ellipsis-2" :class="GET_CONNECT_DEVICEId?'':'gray-text'">{{GET_CONNECT_DEVICEId||'暂未连接打印机'}}</text>
					<text class="status" v-if="GET_CONNECT_DEVICEId">已连接</text>
				</view>
			</view>
			<view class="r" @click="goConnectBle" >
				<view class="rimgBox" >
					<image src="../../../static/img/jiechu.png" mode=""></image>
				</view>
				<view class="text">
					{{GET_CONNECT_DEVICEId?'去更换':'去连接'}}
				</view>
			</view>
		</view> -->
		<!--收寄件信息  -->
		...
		<!-- 	<view class="editTips" v-if="isEdit">
			...
			<view class="bot-wraper">
				...
				<view class="bot-r" >
					<!-- <view class="submit-button print" :class="disable?'disable':''" @tap="submit(0)" >下单并打印</view>
					<view class="submit-button" :class="disable?'disable':''"  @tap="submit(1)">立即下单</view> -->
					<view class="submit-change" @tap="changeSub" v-if="!isEdit">更改偏好</view>
				</view>
				<view class="bot-r">
					<view class="edit-button" :class="disable?'disable':''"  @tap="submit(1)">{{isEdit?'修改': titleX}}</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters([GET_CONNECT_DEVICEId])
		},
		components:{
			SendReceive,
			GoodsInfo,
		...
		methods: {
			...
			checkDispByCity(){
				let self = this;
				let city = this.addressDataSJ?this.addressDataSJ.city:false
				if(city){
					let data = {city:this.addressDataSJ.city};
...
.send{
	...
	.bot-wraper{
		...
		.bot-r{
			...
			.edit-button.disable,.submit-button.disable,.submit-button.print.disable{
