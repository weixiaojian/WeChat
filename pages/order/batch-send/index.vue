<template>
	<view class="batch-send">
				...
				<scroll-view scroll-y="true">
					<block>
						<view class="orderRec" v-for="(item, index) in addressDataSJ" :key="index">
							<!-- 收件信息 -->
							<view class="mask" v-if="item.noPerfect" @click="newaddress('sjr', index)">
								<view class="text">
									地址信息不完整，请完善>>
								</view>
							</view>
							...
							<!-- 物品信息 -->
							<view class="orderRec-shop" >
								<view class="rec-row line" v-if="isProductType">
									<view class="name">产品类型:</view>
									<view class="fl-end" >
										<view class="content">
											<picker  @change="productTypeChange" :data-index="index" style="font-size: 28rpx;" mode = selector :range="typeList" range-key="name">
												<view class="text">
													{{item.goodsInfo.productType||'请选择'}}
												</view>
											</picker>
										</view>
								...
								<view class="rec-row" @click.stop="getGoodsInfo(index,'wpxx')">
									<view style="flex: 2;">
					...
					<view class="addRecives">
						...
						<view class="wrapper two" @tap.stop="goParsingBatch">
							<image class="batchIcon" src="../../../static/img/batchIcon.png" mode=""></image>
							<text class="add" >批量识别地址</text>
						</view>
					</view>
					<!-- 支付方式 -->
		...
		<!-- 			<view class="orderRec-row" v-if="isProductType">
						<view class="name">产品类型:</view>
						<view class="content">
							<picker  @change="productTypeChange"  style="font-size: 28rpx;" mode = selector :range="typeList" range-key="name">
								<view class="text">
									{{productType||'请选择'}}
								</view>
							</picker>
		...
		mounted() {
					...
					this.blurValue();
					 ...
					 _data.forEach(item =>{
						 ...
						 obj.goodsInfo = {
							...
							remark : '',
							goodsPayment :'现金',
							productType:this.isProductType?'当日件':''
						}
			...
			uni.$on('inputAddress',address =>{
				if(this.newaddressId == 'jjr'){
					...
					self.addressDataSJ[this.editRecIndex].noPerfect=false
				}
			});
			uni.$on('batchRec',order =>{
				this.backBatch =false;
				this.echoRecDataByParsing(order)
			});
		...
		methods: {
			...
			echoRecDataByParsing(order){
				...
				order.forEach((item, index) => {
					...
					if(!item.province||!item.cityName||!item.countyName||!item.formatAddress||!this.$Reg.RegTel(item.phone)){
						obj.noPerfect=true
					}
					console.log('obj',obj)
					tempArray.push(obj)
				})
				this.addressDataSJ = this.addressDataSJ.concat(...tempArray);
			},
			goParsingBatch(){
				uni.navigateTo({
			...
			getBlProductType(){
				...
				self.$api.getBlProductType(_data).then(res =>{
			...
			checkDispByCity(){
				let self = this; 
					this.addressDataSJ.forEach((item,index) =>{
						let data = {city:item.address.city};
						item.disable = false;
						self.$api.checkDispByCity(data).then(res=>{
							if(res.code == 400){
								item.disable = true;
			...
			submit(){
				...
				let isNoPerfect = this.addressDataSJ.some(item => item.noPerfect==true);
				if(isNoPerfect){
					uni.showToast({
						icon: "none",
						title: "请完善地址信息",
				...
				timer = setTimeout(()=>{
					...
					for(let i=0;i<this.addressDataSJ.length;i++) {
						if(this.addressDataSJ[i].disable){
							continue;
						}
						...
						if (this.addressDataSJ[i].goodsInfo.goodsWeight == '') {
							uni.showToast({
								icon: "none",
						...
						if (this.addressDataSJ[i].goodsInfo.goodsName == '') {
							uni.showToast({
								icon: "none",
						...
						if (this.addressDataSJ[i].goodsInfo.packingPiece == '') {
							uni.showToast({
								icon: "none",
								title: "请填写第"+(i+1)+"个数量",
								duration:2000
						...
						if (this.addressDataSJ[i].goodsInfo.productType == ''&&this.isProductType) {
							uni.showToast({
								icon: "none",
								title: "请填写第"+(i+1)+"产品类型",
								duration:2000
						...
						_data = {
							...
							productType: this.addressDataSJ[i].goodsInfo.productType, // 产品类型
							originalBillCode: "",
								...
								if(res.data.errorList.length>0){
									let order = res.data.errorList;
										...
										let tempArray = []
									 	...
									 	this.addressDataSJ = tempArray
										console.log(this.addressDataSJ)
								}else{
									setTimeout(() => {
										self.reset_order()
			...
			// 清空缓存
			reset_order(){
				this.addressDataSJ = this.addressDataSJ.filter(item => item.disable);
				this.addressDataJJ = {};
				this.freight = '';
