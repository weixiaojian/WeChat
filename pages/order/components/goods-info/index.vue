<template>
	<view class="goods-info">
		<view class="goods-wraper">
			...
			<view class="row" v-if="isProductType||editProductType">
					<view class="name">产品类型:</view>
					<view class="content">
						<picker   @change="productTypeChange" :disabled="!isProductType" style="font-size: 28rpx;" mode = selector :range="typeList" range-key="name">
								<view class="text">
										{{productType||'请选择'}}
								</view>
						</picker>
	...
	export default {
		...
		watch:{
		     payment_type(val, oldVal){//普通的watch监听
				 ...
				 isProductType(val, oldVal){//普通的watch监听
				 console.log('val',val)
