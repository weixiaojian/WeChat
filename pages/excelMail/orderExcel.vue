			...
			<block v-if="currentId == 'excel'">
				<view class="orderRec" v-for="(item, index) in orderList" :key="index">
					...
					<view class="orderRec-shop" >
						...
						<view class="rec-row" @click.stop="changeShop('wpxx', index)">
							<view style="flex: 2;">
				...
				if(this.currentId == 'excel') {
					...
					if (this.orderList[i].productType == ''&&this.isProductType) {
						...
						_data = {
							...
							productType: this.productType, // 时效件
							originalBillCode: "",
					...
					for(let i=0;i<this.batchList.length;i++) {
						...
						if (this.batchList[i].productType == ''&&this.isProductType) {
							uni.showToast({
								icon: "none",
								title: "请填写第"+(i+1)+"产品类型",
								duration:2000
						...
						_data = {
							...
							acceptManAddress:this.$Reg.stripscript(this.batchList[i].formatAddress), // 收件详细地址
